...
public class AllAnagrams
{
    ...
    throws IOException 
  {
    ...
    HashTable<LinkedList<String>> hash = new HashTable<>(expectedNumWords); 
    ...
    while(scin.hasNext())
    {
	  ...
	  {
		  list = hash.get(sortedWord);
	  }
	  else
	  {
		  list = new LinkedList<String>();
		  hash.set(sortedWord, list);
	  }
	  list.addLast(word);
	...
	{
		LinkedList<String> list = hash.get(keys[i]);
		if (list.size() > 1)
		{
			anagrams.addLast(list);
		}
	}
